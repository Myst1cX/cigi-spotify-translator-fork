// ==UserScript==
// @name         Cigi Spotify Translator 1.11
...
// ==/UserScript==
// Changelog:
// 1.1 - added toggleable debug logging (Tampermonkey menu command, off by default, ✓/✗ Debug Logging)
// 1.2 - fixed Spotify DOM change: [data-testid="fullscreen-lyric"/"lyrics-container"] retired,
//       lines now self-identify via [data-testid="lyrics-line"] (per findings doc).
//       Removed 2s polling (checkForTranslation) and MutationObserver auto-retranslate (observeLyrics) -
//       script now does nothing until Translate is clicked. Click-while-translating is now a state
//       machine: same language while a run is in flight = ignored (let it finish); different language
//       while a run is in flight = old run's results are discarded via a generation counter, lines are
//       cleared, new run starts. Language dropdown selection no longer auto-triggers translation.
// 1.3 - fixed translations disappearing on synced-highlight/seek: Spotify's lyrics view is React-owned,
//       so when a line's active/highlight state changes (playback advancing OR user clicking to seek),
//       React re-renders that line's children and wipes our foreign data-translated div even though
//       the translation itself never changed. Now caches original-text -> translated-text pairs after
//       each successful run and watches (via MutationObserver, repair-only) for lines missing their
//       translation, reinserting from cache with zero network calls. Observer is rebuilt fresh on every
//       new translateLyrics() run and disconnected at the start of the next one, so it never touches
//       requests, generations, or the click state machine from 1.2.
// 1.4 - waitForLyrics() renamed watchLyricsPresence() and made persistent instead of one-shot: header
//       now mounts on lyrics-open and unmounts (fully removed from DOM) on lyrics-close, re-detecting
//       either transition every 1s. On close, any in-flight translation is aborted via the existing
//       generation counter (prevents writing into DOM nodes Spotify already tore down), repair observer
//       is disconnected, cache cleared. Header is now position:sticky/top:0/z-index:9999 so it stays
//       pinned to the top of the scroll node regardless of scrolling, line navigation, or DOM churn.
// 1.5 - position:sticky confirmed broken (header stayed in normal flow, scrolled away with content) -
//       almost certainly OverlayScrollbars applying a transform to an ancestor, which gives sticky a
//       new containing block it can't escape. Replaced with position:fixed, appended directly to
//       document.body (sidesteps that ancestor entirely) and rect-synced to .main-view-container's
//       getBoundingClientRect() - same source element Web Lyrics+ uses for its popup positioning.
//       An in-flow spacer (matching header height) is inserted where the header used to sit so lyric
//       lines don't render underneath it. Position/size are resynced via a ResizeObserver on
//       .main-view-container (catches sidebar collapse/NPV open, which don't fire window resize) plus
//       a window resize listener as a fallback. Both are cleaned up in removeHeader() on lyrics-close.
// 1.6 - added a persistent, cross-session, cross-language translation cache (PersistentCache), stored
//       via GM_setValue/GM_getValue (confirmed synchronous, no await needed) under key
//       "cigiTranslationCache_v1", keyed on a composite `${trackId}::${langCode}` string so multiple
//       languages can be cached per song. Song identity prefers the real 22-char Spotify trackId read
//       off the [data-testid="context-link"] anchor's href (same selector Web Lyrics+ uses), falling
//       back to a title-artist string if that anchor isn't present. translateLyrics() now checks the
//       cache as its first step: on a hit, skips translateText()/network entirely and inserts
//       translations by matching {original, translated} pairs against current lyric line text (same
//       by-text matching the v1.3 repair observer already uses), then populates the existing in-memory
//       lineTranslationCache exactly as a fresh run would so the repair observer needs no special-casing;
//       on a miss, existing network flow is unchanged except the result is now also written into the
//       persistent cache. Cache hits log an unconditional console.log (mirroring Web Lyrics+'s pattern)
//       plus the usual gated dbgLog() detail. Eviction is count-based (oldest-timestamp-first, LRU
//       touch on every hit) rather than byte-based, since GM storage isn't sharing Spotify's page-origin
//       localStorage quota. Two new one-shot Tampermonkey menu commands, separate from the existing
//       Debug Logging toggle: "Cigi: Show Translation Cache" (console.table, 1-indexed) and
//       "Cigi: Clear Translation Cache" (confirm -> clear -> confirm). No user-facing UI indicator for
//       cache hits by design - debug console only. getCurrentSongIdentity() now also dbgLogs its
//       success path (resolved trackId), not just the fallback/failure branches. PersistentCache.set()
//       now distinguishes a brand-new entry from an overwrite of an existing one in its dbgLog line.
//       Show Translation Cache's table no longer carries a redundant manual "#" column alongside
//       console.table's own auto index - rows are now keyed 1..n directly so the built-in (index)
//       column is the 1-indexed row number, with no duplicate/off-by-one column. Added a small
//       folder+checkmark SVG icon next to the Translate button, shown only on a cache-hit render
//       (opacity-faded in via CSS transition, auto-fades back out after ~2s via a single tracked
//       timeout) - nothing shown on a cache-miss/fresh network translation. This is a per-render
//       signal, not per-line: cache-hit/miss is decided once per translateLyrics() run, so every
//       line in a given render shares the same origin. Icon element and its timeout are cleaned up
//       in removeHeader() same as the rest of the header's state.
// 1.7 - split the single cache icon into two distinct signals, since "just got cached" and "pulled
//       from cache" are different events worth telling apart at a glance. The existing folder+
//       checkmark icon (showNewlyCachedIcon()) is repurposed: it now fires at the moment a fresh
//       network translation is actually stored into PersistentCache (end of the cache-miss branch,
//       right after PersistentCache.set()), not on a hit. A new database+magnifier+checkmark icon
//       (showCacheHitIcon(), cacheHitIconElement/cacheHitIconHideTimeout) is added for the actual
//       cache-hit case (lyrics pulled from cache, network skipped) - same size (20x20 viewBox 24),
//       same opacity-fade/2s-timeout/cleanup logic as the original icon, just a separate element/
//       timeout so the two can't interfere with each other if both fire in quick succession (e.g.
//       rapid language switching between a cached and an uncached language). Both icons still fire
//       silently on nothing else - no icon at all if a run is superseded before it finishes.
// 1.8 - fixed the cache-hit (database) icon rendering misaligned and half-glitched next to the
//       folder icon. Root cause: several of the cylinder's band shapes were open paths with a fill
//       applied - SVG auto-closes an open filled path with a straight line back to its start point,
//       which was carving stray diagonal wedges into the cylinder instead of clean rounded bands,
//       and threw off its visual footprint versus the folder icon's tighter symmetric one. Rebuilt
//       the icon from closed primitives only (<rect>, <ellipse>) for every filled shape; the two
//       horizontal divider lines between cylinder bands are stroke-only (fill="none") paths, so an
//       open path there is harmless. Success badge (green circle + checkmark) is now copy-pasted
//       verbatim from the folder icon's markup, same cx="18" cy="18" r="4.2" and same checkmark path,
//       so both icons' badges land in the exact same spot when swapped in the header. Dropped the
//       separate magnifying-glass glyph (it was contributing to the offset/clipping); the shared
//       checkmark badge alone still reads clearly as "success," same as the folder icon. Cylinder
//       recolored to light blue body / light tan rim to read as a matched pair with the folder icon's
//       palette. No changes to when/why the icon fires - still showCacheHitIcon(), still fired only
//       on an actual cache hit, still cleaned up in removeHeader().
// 1.9 - fixed the two cache icons rendering in different positions from each other. They were
//       separate flex children of controlsContainer (folder icon then database icon, `gap: 15px`
//       apart); opacity:0 hides pixels but still reserves layout space, so each icon always sat in
//       its own slot and whichever one was visible appeared one `gap` further right/left than the
//       other, also nudging the whole centered row's effective width. Both icons are now appended
//       into a single new `iconSlot` wrapper (one flex child, `position: relative`, sized to the
//       20x20 icon box) and each icon is `position: absolute; top: 0; left: 0` inside it, so they
//       overlap in exactly the same spot instead of occupying two adjacent slots. No change to
//       showNewlyCachedIcon()/showCacheHitIcon() or their opacity/timeout logic - purely a layout fix.
// 1.10 - Debug Logging menu label now uses ✅/❌ emoji instead of the plain ✓/✗ glyphs, for a
//        clearer at-a-glance state in the Tampermonkey menu. Cosmetic only, in registerDebugMenu();
//        toggleDebugLogging()'s own console.log confirmation text is unchanged.
// 1.11 - Debug Logging menu label now reads "Debug Logging (Console)" to make it clear the toggle
//        controls console output, not some other kind of logging. Cosmetic only, in
//        registerDebugMenu(); toggleDebugLogging()'s behavior and confirmation console.log unchanged.
//        (No reload needed for this or any debugEnabled toggle: dbgLog() reads the same in-memory
//        `debugEnabled` variable that toggleDebugLogging() flips directly - GM_setValue there is only
//        for persisting the setting to the next page load, GM_getValue at startup is what restores it.)
